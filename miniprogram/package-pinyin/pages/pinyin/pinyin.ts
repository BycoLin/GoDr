...
Page({
  ...
  onTapInitial(e: WechatMiniprogram.TouchEvent) {
    ...
    if (fromPath && pathKind === 'pinyin') {
      ...
      wx.redirectTo({
        url: routePage(
          ROUTES.pinyinLearn,
          `kind=initial&initial=${id}&mode=card&fromPath=pinyin&pathStep=${pathStep || 'learn'}&justLearned=1`,
        ),
      });
      return;
    }
    wx.navigateTo({
      url: routePage(ROUTES.pinyinLearn, `kind=initial&initial=${id}`),
    });
  },
  ...
  onTapFinal(e: WechatMiniprogram.TouchEvent) {
    const id = e.currentTarget.dataset.id as string;
    wx.navigateTo({
      url: routePage(ROUTES.pinyinLearn, `kind=final&final=${encodeURIComponent(id)}&mode=card`),
    });
  },
  ...
  onTapDrill(e: WechatMiniprogram.TouchEvent) {
    ...
    wx.navigateTo({
      url: routePage(ROUTES.pinyinDrill, `kind=${kind}`),
    });
  },
});
