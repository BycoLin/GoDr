...
export function focusRemindContent(elapsedMin = FOCUS_LIMIT_MIN): string {
  return `已经玩一会儿啦（大约 ${elapsedMin} 分钟）。歇歇眼睛、喝口水也很棒。想继续就继续，想休息就先结束～`;
}

function notifyPlayFocusTip() {
  const pages = getCurrentPages();
  const top = pages[pages.length - 1] as WechatMiniprogram.Page.Instance<
    WechatMiniprogram.IAnyObject,
    WechatMiniprogram.IAnyObject
  > & { route?: string; setData?: (d: Record<string, unknown>) => void };
  if (top?.route === 'pages/play/play' && top.setData) {
    top.setData({ focusReminded: true });
  }
}

function tryEndCurrentPlay() {
  const pages = getCurrentPages();
  const top = pages[pages.length - 1] as WechatMiniprogram.Page.Instance<
    WechatMiniprogram.IAnyObject,
    WechatMiniprogram.IAnyObject
  > & { route?: string; onEndFromFocus?: () => void };
  if (top?.route === 'pages/play/play' && top.onEndFromFocus) {
    top.onEndFromFocus();
  }
}

function showFocusRemindModal(mins: number) {
  const app = getApp<IAppOption>();
  app.globalData.focusReminded = true;
  notifyPlayFocusTip();
  wx.showModal({
    title: '休息一下吧',
    content: focusRemindContent(mins),
    confirmText: '继续练',
    cancelText: '先结束',
    success: (res) => {
...
export function triggerFocusRemindIfDue(): void {
  ...
  const mins = Math.max(getFocusLimitMin(), focusElapsedMin(globalSession));
  showFocusRemindModal(mins);
}
